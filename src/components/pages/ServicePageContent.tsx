  ...
  return (
    <>
      ...
      >
        ...
        <motion.p
          initial={h ? { opacity: 0, y: 8 } : { opacity: 1 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          <section style={{ maxWidth: '780px', margin: '0 auto', padding: 'clamp(40px, 5vw, 64px) 24px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
              {relatedServices.map((rs, i) => (
                <Link key={i} href={rs.href} style={{ textDecoration: 'none' }}>
                  <Button variant="secondary" size="sm">{rs.title} &rarr;</Button>
                </Link>
              ))}
            </div>
          </section>
      ...
      <div style={{ marginTop: '24px' }}>
        <Link href="/contact" style={{ display: 'block', maxWidth: '780px', margin: '0 auto', padding: '0 24px 48px', textDecoration: 'none' }}>
          <Button variant="primary" size="lg" fullWidth>Get a Free Estimate &rarr;</Button>
        </Link>
      </div>
